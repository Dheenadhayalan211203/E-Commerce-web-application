...
app.get("/api/products/:id", async (req, res) => {
  try {
    ...
    res.json({
      ...product[0],
      flavors: product[0].flavors_data
        ? JSON.parse(product[0].flavors_data)
        : [],
    });
  } catch (error) {
    console.error("Error:", error);
  ...
  try {
    const cartItems = await Cart.findAll({
      ...
      include: [{
        model: Product,
        as: 'product',
        attributes: ['id', 'name', 'price', 'image_base64', 'stock', 'is_active']
      }]
    });
    ...
    const enrichedCart = cartItems.map(item => ({
      ...item.get({ plain: true }),
      total_price: (item.quantity * item.product.price).toFixed(2)
    }));
...
app.post('/api/cart/add', async (req, res) => {
  try {
    ...
    // Upsert (update or insert) cart item
    const [cartItem] = await Cart.upsert({
      user_id,
      product_id,
      quantity,
    ...
    // Fetch the item with product details
