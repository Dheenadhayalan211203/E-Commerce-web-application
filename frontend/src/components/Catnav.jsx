...
const Catnav = () => {
  ...
  useEffect(() => {
    ...
    if (pathParts[1] === 'category') {
      setActiveCategory(decodeURIComponent(pathParts[2]).toLowerCase());
  ...
  const handleSubcategoryClick = (parentName, subName, e) => {
    ...
    setActiveCategory(subName.toLowerCase());
    navigate(`/category/${encodedName}`);
    setMobileMenuOpen(false);
  };
                ...
                <div className={`subcategory-dropdown ${expandedCategory === cat.id ? 'expanded' : ''}`}>
                  {cat.subcategories?.map(sub => (
                    <Link
                      ...
                      className={`subcategory-link ${activeCategory === sub.name.toLowerCase() ? 'active' : ''}`}
